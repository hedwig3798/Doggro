...
Bowl::Bowl(std::string _name, ManagerSet* _managerSet, OBJECT_TYPE _type, LoadMapData* _objectManager, QuestManager* _questManager, DogScript* _dogScript)
	: Entity(_name, _managerSet, _type)
{
	...
	Vector3 cPosition(aBoundingBox.cx, aBoundingBox.cy+20, aBoundingBox.cz);
	...
	CreateComponent<BowlScript>(this);
	GetComponent<BowlScript>()->GetQuestManager(questManager);
...
void Bowl::Update(float _deltaTime)
{
