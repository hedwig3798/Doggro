...
WallCollider2Stage2::WallCollider2Stage2(std::string _name, ManagerSet* _managerSet, OBJECT_TYPE _type, LoadMapData* _objectManager)
	:Entity(_name, _managerSet, _type)
{
	...
	GetComponent<Transform>()->SetPosition(Vector3(400.f, 150.f, 0.f));
	...
	GetComponent<StaticCollider>()->SetSize(Vector3(10.f, 900.281f, 400.f));
	GetComponent<StaticCollider>()->Start();
}

WallCollider2Stage2::~WallCollider2Stage2()
{
