...
Leash::Leash(std::string _name, ManagerSet* _managerSet, OBJECT_TYPE _type, LoadMapData* _objectManager, QuestManager* _questManager, DogScript* _dogScript)
	: Entity(_name, _managerSet, _type)
{
	...
	Vector3 cPosition(aBoundingBox.cx-100, aBoundingBox.cy, aBoundingBox.cz);
	...
	CreateComponent<LeashScript>(this);
	GetComponent<LeashScript>()->GetQuestManager(questManager);
...
void Leash::Update(float _deltaTime)
{
